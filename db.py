...
def save_result(full_name, stu_numb, group_name, average_mark):
    try:
        conn = pymysql.connect(host='localhost',
                               user='root',
                               password='root',
                               ...
                               cursorclass=pymysql.cursors.Cursor)
        cursor = conn.cursor()
        cursor.execute("INSERT INTO " + NameDB.namedb.get_name_tb() + f"(FullName, StuNumb, GroupName,AverageMark) "
                                                                      f"VALUES (%s, %s, %s, %s)",
                       (str(full_name), str(stu_numb), str(group_name), str(average_mark)))
        conn.commit()
...
def save_db_to_xlsx():
    try:
        conn = create_engine('mysql+pymysql://root:root@localhost/' + NameDB.namedb.get_name_db())
